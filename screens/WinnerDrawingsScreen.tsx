...
import { View, Text, FlatList, Image, StyleSheet, Modal, TouchableWithoutFeedback, TouchableOpacity as RNTouchableOpacity, Dimensions } from "react-native";
...
const handleShare = (item: Winner) => shareDrawing(item);
      ...
      try {
        ...
        if (!querySnapshotNext.empty) {
          ...
          querySnapshotNext.forEach((doc) => {
            const data = doc.data();
            const date = data.date instanceof Timestamp ? data.date.toDate() : "";
            newWinners.push({ id: doc.id, image: data.image, imageUrl: data.imageUrl, date });
          });
